...
Daemon that finds the sunrise, sunset and solar noon times for a given location (defined by environment variables) and automatically takes photos at those times, by using `fswebcam` (must be installed) and uploading it to a Google Cloud Storage bucket.
...
- `FSWEBCAM_*` - Options for `fswebcam`, see `src/config.ts`
